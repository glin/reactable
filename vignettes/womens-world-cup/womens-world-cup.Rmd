...
hex_color_ramp <- function(colors, bias = 1) {
  get_color <- colorRamp(colors, bias = bias)
  function(x) rgb(get_color(x), maxColorValue = 255)
}

off_rating_color <- hex_color_ramp(c("#ff2700", "#f8fcf8", "#44ab43"), bias = 1.3)
def_rating_color <- hex_color_ramp(c("#ff2700", "#f8fcf8", "#44ab43"), bias = 0.6)
knockout_pct_color <- hex_color_ramp(c("#ffffff", "#f2fbd2", "#c9ecb4", "#93d3ab", "#35b0ab"), bias = 2)

rating_column <- function(maxWidth = 45, ...) {
  ...
  columns = list(
    team = colDef(
      ...
      cell = function(value, index) {
        div(
          class = "team",
          img(class = "flag", alt = "", src = sprintf("images/%s.png", value)),
    ...
    group = colDef(defaultSortOrder = "asc", align = "center", class = "group", headerStyle = list(fontWeight = 700)),
    spi = rating_column(format = colFormat(digits = 1), maxWidth = 55),
    ...
    make_semis = knockout_column(name = "Make Semifinals", maxWidth = 85),
  ...
  rowClass = JS("
    function(rowInfo, state) {
      ...
      if (firstSorted && firstSorted.id === 'group') {
        const nextRow = state.sortedData[rowInfo.viewIndex + 1]
