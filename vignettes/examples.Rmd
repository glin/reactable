...
for more details on how to use render functions.
...
R render functions support conditional rendering. If the render function
...
You can set `details` on individual columns, and even show multiple details for a row:
```{r}
reactable(iris[1:5, ],
...
that returns custom content. R render functions support `htmltools` and
`htmlwidgets`, while JavaScript render functions allow for more dynamic behavior.

See [Custom Rendering](custom-rendering.html) for more details on how to
...
[Custom Rendering](custom-rendering.html) for more details on how to
...
## Conditional Styling
You can style cells and rows conditionally using an R or JavaScript
function. Both `style`/`class` and `rowStyle`/`rowClass` accept functions
with the same arguments as their custom render function counterparts
(see [Custom Rendering](custom-rendering.html) for details).

> **NOTE:** R functions can optionally return inline styles as named
> lists with camelCased property names. JavaScript functions, however,
> must return inline styles as objects with camelCased property names.
...
You can customize table styling using several options that can all be combined:
