...
### Exclude sub rows from pagination

> New in v0.2.3.9000

Sub rows are paginated and included in the row count by default, but you can
exclude sub rows by setting `paginateSubRows` to `FALSE`. This will show all
expanded sub rows on the same page, so it is not recommended for tables with
large row groups.

```{r}
data <- MASS::Cars93[, c("Manufacturer", "Model", "Type", "Price", "MPG.city")]

reactable(data, groupBy = "Type", paginateSubRows = FALSE)
```
