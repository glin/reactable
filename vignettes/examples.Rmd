...
You can enable virtual scrolling by setting `virtual = TRUE`. Virtual scrolling renders only the visible rows, improving performance for large datasets. You should set a fixed `height` when using virtual scrolling so the table has a scroll container.

```{r}
# Create a large dataset
data <- data.frame(
  row = 1:10000,
  x = round(rnorm(10000), 4),
  y = round(runif(10000), 4)
)

reactable(data, pagination = FALSE, virtual = TRUE, height = 500)
```

Virtual scrolling can be combined with pagination. When both are enabled, only visible rows within the current page are rendered:
...
Virtual scrolling also works with grouped tables and expandable row details:

```{r}
data <- MASS::Cars93[, c("Type", "Manufacturer", "Model", "Price", "MPG.city")]
...
reactable(
  ...
  groupBy = "Type",
  columns = list(
    Price = colDef(aggregate = "mean", format = colFormat(digits = 1)),
    MPG.city = colDef(aggregate = "mean", format = colFormat(digits = 1))
  ),
  details = function(index) {
    paste("Details for row", index, "- Model:", data$Model[index])
  }
)
```

::: {.callout-note}
**Note:** Browser find-in-page (Ctrl+F / Cmd+F) does not work with virtualized tables since only visible rows exist in the DOM. Use the table's built-in search or filtering instead.
:::
