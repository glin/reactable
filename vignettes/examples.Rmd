...
`"desc"` (descending):
```{r}
reactable(iris[48:52, ], defaultSortOrder = "desc", columns = list(
...
Missing values are shown as empty cells by default, but you can customize their
display text by setting `na` on a column:
```{r}
reactable(
  data.frame(
    n = seq(5),
...
You can customize the table style using several options that can be mixed and matched:
