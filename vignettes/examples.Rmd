...
You can set default expanded rows by specifying a vector of row indices in `defaultExpanded`:
```{r}
reactable(
  iris[1:4, ],
  details = function(index) paste("Details for row:", index),
  defaultExpanded = c(1, 3)
)
```

When there are multiple row details, you can specify which columns are expanded
using a named list instead:
```{r}
reactable(
  iris[1:4, ],
  details = function(index) paste("Details for row:", index),
  columns = list(Species = colDef(details = function(index) paste("Species details for row:", index))),
  defaultExpanded = list(Species = c(1, 2, 3), .details = 3)
)
```

> Note: to specify table-level row details in a named list, use `.details` as the column name.
