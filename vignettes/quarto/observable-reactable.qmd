...
:::{.callout-note}
These examples require new features in reactable v0.3.0.9000 (unreleased).
[Install reactable from GitHub](https://github.com/glin/reactable/#installation) to try them out.
:::

Source: [observable-reactable.qmd](https://github.com/glin/reactable/blob/main/vignettes/quarto/observable-reactable.qmd)
...
filteredData = Generators.observe(next => {
  Reactable.onStateChange('tbl-input', state => {
    next(state.sortedData)
  })
})
```
