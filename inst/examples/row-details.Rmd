...
```{r}
bySpecies <- aggregate(iris[, 1:4], by = list(Species = iris$Species), FUN = mean)

reactable(bySpecies, details = function(index) {
  species <- bySpecies[index, "Species"]
  htmltools::div(
