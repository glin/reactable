...
### Virtual scrolling with no rows found

Expected: Filtering out all rows results in the "No rows found" message in the first row, and pagination showing a single page

```{r}
reactable(data_1k, virtual = TRUE, pagination = FALSE, height = 500, showPagination = TRUE, searchable = TRUE)
```

### Virtual scrolling with no rows found and minRows

Expected: Filtering out all rows results in the "No rows found" message in the middle of the table, and pagination showing a single page

```{r}
reactable(data_1k, virtual = TRUE, pagination = FALSE, minRows = 10, showPagination = TRUE, searchable = TRUE)
```
...
Server-side data combined with virtual scrolling is useful when server pages are large (e.g., 1000 rows) but only ~15 fit on screen:
...
Currently all 100 columns are rendered even with `virtual = TRUE`. Column virtualization would render only visible columns.
...
Instead of page-based fetching, the server would send only the visible rows based on scroll position. This would allow seamless scrolling through millions of rows without pagination controls.
